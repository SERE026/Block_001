$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sport/traingoal/list',
        datatype: "json",
        colModel: [			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.trainGoal.id == null ? "sport/traingoal/save" : "sport/traingoal/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.trainGoal),
                    success: function(r){
                        if(r.code === 0){
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "sport/traingoal/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sport/traingoal/info/"+id, function(r){
